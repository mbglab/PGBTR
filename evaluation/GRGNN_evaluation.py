# Note: You should install GRGNN from https://github.com/juexinwang/GRGNN before use this code.
# Usage: python Main_inductive_ensemble.py --traindata-name Subtiwiki_Bsubtilis --testdata-name Subtiwiki_Bsubtilis
# Here, for convenience, "traindata-name" and "testdata-name" are set to be the same. In actual operation, the divided data set is used.
...
'''Prepare data'''
# If you want to evaluate other datasets you should change the path here.
# The data will be acquired after run the PGBTR_evaluation.py, that can
# make sure that all the comparisions can conduct on the same dataset.
mydata_path = '../data/Subtiwiki_Bsubtilis/processed_balanced/'
result_file = 'result/Subtiwiki_Bsubtilis/result_balanced_Subtiwiki_Bsubtilis.txt'

args.file_dir = os.path.dirname(os.path.realpath('__file__'))
# The counts of TF shuold be change if you change the dataset to evaluate.
dreamTFdict['Subtiwiki_Bsubtilis']=198
...
for iter_num in range(1,11):
    ...
    for num in range(5):
        ...
        if args.traindata_name is not None:
            ...
            # Prepare Training
            trainGroup = np.load(os.path.join(args.file_dir, 'data/Subtiwiki_Bsubtilis/ind.{}.allx'.format(trdata_name)),allow_pickle=True)
            # Pearson's correlation/Mutual Information as the starting skeletons
            trainNet_agent0 = np.load(args.file_dir+'/data/Subtiwiki_Bsubtilis/'+trdata_name+'_pmatrix_'+str(args.pearson_net)+'.npy',allow_pickle=True).tolist()
            trainNet_agent1 = np.load(args.file_dir+'/data/Subtiwiki_Bsubtilis/'+trdata_name+'_mmatrix_'+str(args.mutual_net)+'.npy',allow_pickle=True).tolist()
            ...
            # Prepare Testing    
            testGroup = np.load(os.path.join(args.file_dir, 'data/Subtiwiki_Bsubtilis/ind.{}.allx'.format(tedata_name)),allow_pickle=True)
            # Pearson's correlation/Mutual Information as the starting skeletons
            testNet_agent0 = np.load(args.file_dir+'/data/Subtiwiki_Bsubtilis/'+tedata_name+'_pmatrix_'+str(args.pearson_net)+'.npy',allow_pickle=True).tolist()
            testNet_agent1 = np.load(args.file_dir+'/data/Subtiwiki_Bsubtilis/'+tedata_name+'_mmatrix_'+str(args.mutual_net)+'.npy',allow_pickle=True).tolist()
            # Random network as the starting skeletons
            ...
            train_pos, train_neg = read_data(mydata_path + f'/{iter_num}/train_data{num}.txt')

            test_pos, test_neg = read_data(mydata_path + f'/{iter_num}/test_data{num}.txt')
        ...
        # Agent 0
        _, _, test_neg_agent0,  _,test_prob_agent0 =DGCNN_classifer(train_graphs_agent0, test_graphs_agent0, train_node_information_agent0, max_n_label_agent0, set_epoch = 70, eval_flag=True)
        # Agent 1
        _, _, test_neg_agent1,  _,test_prob_agent1 =DGCNN_classifer(train_graphs_agent1, test_graphs_agent1, train_node_information_agent1, max_n_label_agent1, set_epoch = 70, eval_flag=True)
        ...
        precision, recall, thresholds = precision_recall_curve(trueList, ensembleProb)
        aupr = auc(recall, precision)
        
        allstr     = str(float((tp+tn)/len(test_graphs_agent0)))+"\t"+str(tp)+"\t"+str(len(test_pos[0])-tp)+"\t"+str(tn)+"\t"+str(len(test_neg[0])-tn)+"\t"+str(roc_auc_score(trueList, ensembleProb))+"\t"+str(aupr)
        ...
        print("Ensemble:Accuracy tp fn tn fp AUC AUPR")
        print(allstr+"\n")
        ...
        print(agent1_str+"\n") 
        # 计算召回率（Recall）
        fn = len(test_pos[0])-tp
        fp = len(test_neg[0])-tn
        recall = tp / (tp + fn)
        ...
        auroc_list.append(roc_auc_score(trueList, ensembleProb))
    ...
    final_aupr.append(np.mean(aupr_list))
    final_precision.append(np.mean(precision_list))
    final_recall.append(np.mean(recall_list))
    final_f1score.append(np.mean(f1score_list))

with open(result_file, 'w') as f:
    f.write(f'AUROC: {str(final_auroc)}   mean: {str(np.mean(final_auroc))}±{str(np.std(final_auroc))}'+ '\n')
    f.write(f'AUPR: {str(final_aupr)} mean: {str(np.mean(final_aupr))}±{str(np.std(final_aupr))}'+'\n')    
    f.write(f'precision: {str(final_precision)}   mean: {str(np.mean(final_precision))}'+ '\n')
    f.write(f'recall: {str(final_recall)}   mean: {str(np.mean(final_recall))}'+ '\n')
    f.write(f'f1-score: {str(final_f1score)}   mean: {str(np.mean(final_f1score))}±{str(np.std(final_f1score))}'+ '\n')    
